...
import coffee1993.nearchat.crashManager.CrashHander;
...
public class NearChatApplication extends Application {
    ...
    private static NearChatApplication instance;
     ...
     * @return instance
     */
    public static NearChatApplication getInstance() {
        return instance;
    }
...
*/        
    ...
    public void onLowMemory() {
        super.onLowMemory();
        NearChatLog.e("BaseApplication", "onLowMemory");
    }

    @Override
    public void onTerminate() {
        super.onTerminate();
        NearChatLog.e("BaseApplication", "onTerminate");
    }
    ...
    private void initFolder() {
        if (null == IMAG_PATH) {
            ...
            if (!FileUtils.isFileExists(IMAG_PATH))
                FileUtils.createDirFile(NearChatApplication.IMAG_PATH);
            if (!FileUtils.isFileExists(THUMBNAIL_PATH))
                FileUtils.createDirFile(NearChatApplication.THUMBNAIL_PATH);
            if (!FileUtils.isFileExists(VOICE_PATH))
                FileUtils.createDirFile(NearChatApplication.VOICE_PATH);
            if (!FileUtils.isFileExists(FILE_PATH))
                FileUtils.createDirFile(NearChatApplication.FILE_PATH);
