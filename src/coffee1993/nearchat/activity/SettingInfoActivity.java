...
public class SettingInfoActivity extends BaseActivity implements OnClickListener,
    ...
    private void initData() {
        mAge = SessionManager.getAge();
        mAvatar = SessionManager.getAvatar();
        mGender = SessionManager.getGender();
        mConstellation = SessionManager.getConstellation(); // 星座
        mBirthday = SessionManager.getBirthday();
        mSelectDate = DateUtil.getDate(mBirthday);
        ...
        Picasso.with(mContext).load(ImageUtils.getImageID(NearUser.AVATAR + mAvatar)).into(mIvAvater);
        mEtNickname.setText(SessionManager.getNickname());
    }
        ...
        putAsyncTask(new AsyncTask<Void, Void, Boolean>() {
            ...
            protected Boolean doInBackground(Void... params) {
                try {
                    ...
                    // 设置用户Session信息
                    SessionManager.setNickname(mNickname);
                    SessionManager.setBirthday(mBirthday);
                    SessionManager.setAge(mAge);
                    SessionManager.setGender(mGender);
                    SessionManager.setAvatar(mAvatar);
                    SessionManager.setConstellation(mConstellation);
                    ...
                    mEditor.putString(NearUser.NICKNAME, mNickname).putString(NearUser.GENDER, mGender)
                            .putInt(NearUser.AVATAR, mAvatar).putInt(NearUser.AGE, mAge)
                            .putString(NearUser.BIRTHDAY, mBirthday)
                            .putString(NearUser.CONSTELLATION, mConstellation);
                    mEditor.commit();
                    return true;
                }
                catch (Exception e) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == REQUEST_CODE) {
            if (resultCode == RESULT_OK) {
                ...
                mAvatar = result + 1;
                Picasso.with(mContext).load(ImageUtils.getImageID(NearUser.AVATAR + mAvatar))
                        .into(mIvAvater);
            }
        }
