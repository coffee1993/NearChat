...
public abstract class BaseMessageActivity extends BaseActivity implements 
    ...
    protected Button mBtnTextDitoraudioSend;
    ...
    protected List<MsgEntity> mMessagesList; // 消息列表
    protected ChatAdapter mAdapter;
    protected NearUser mChatUser; // 聊天的对象
    protected String mCameraImagePath;
    protected SqlDBManager mSqlDBManager;
   ...
   /* protected void initRounds() {
    ...
    protected void initfolder() {
        if (null != NearChatApplication.IMAG_PATH) {
            String imei = mChatUser.getIMEI();
            IMAG_PATH = NearChatApplication.IMAG_PATH + File.separator + imei;
            THUMBNAIL_PATH = NearChatApplication.THUMBNAIL_PATH + File.separator + imei;
            VOICE_PATH = NearChatApplication.VOICE_PATH + File.separator + imei;
            FILE_PATH = NearChatApplication.FILE_PATH + File.separator + imei;
    ...
    public void sendMessage(String content, CONTENT_TYPE type) {
        ...
        MsgEntity msg = new MsgEntity(mIMEI, nowtime, content, type);
        mMessagesList.add(msg);
        mEventHub.addLastMsgCache(mChatUser.getIMEI(), msg); // 更新消息缓存

        switch (type) {
            case TEXT:
                mEventHub.sendUDPdata(MSGProtocolConst.IPMSG_SENDMSG, mChatUser.getIpaddress(),
                        msg);
                break;

            case IMAGE:
            	
            	NearChatLog.i("BaseActivity IMAGE send");
                mEventHub.sendUDPdata(MSGProtocolConst.IPMSG_REQUEST_IMAGE_DATA,
                        mChatUser.getIpaddress(),null);
                break;

            case VOICE:
                mEventHub.sendUDPdata(MSGProtocolConst.IPMSG_REQUEST_VOICE_DATA,
                        mChatUser.getIpaddress(),null);
                break;

            case FILE:
                MsgEntity fileMsg = msg.clone();
        ...
        mSqlDBManager.addChattingInfo(mID, mSenderID, nowtime, content, type);
    }
