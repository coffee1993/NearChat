...
import coffee1993.nearchat.BaseObjectListAdapter;
...
import coffee1993.nearchat.view.EmoticonsTextView;
...
public class ChatAdapter extends BaseObjectListAdapter {
    ...
    public void setChatUser(NearUser user) {
        this.mChatUser = user;
    }

    @Override
    public View getView(int position, View convertView, ViewGroup parent) {
        final MsgEntity msg = (MsgEntity) getItem(position); //一个Item为一个Message
        if (SessionManager.isLocalUser(msg.getSenderIMEI())) {
            String avatarFileName = NearUser.AVATAR + SessionManager.getAvatar();
            mAvatarId = ImageUtils.getImageID(avatarFileName);
        }
        else {
            NearUser users = mUDPListener.getOnlineUser(msg.getSenderIMEI());
            String avatarFileName = NearUser.AVATAR + users.getAvatar();
            mAvatarId = ImageUtils.getImageID(avatarFileName);
        }
    ...
    public int getItemViewType(int position) {
        ...
        MsgEntity msg = (MsgEntity) getItem(position);
        int type = -1;
        if (SessionManager.isLocalUser(msg.getSenderIMEI())) {
    ...
    private void updateView(android.os.Message paramMsg) {
        ...
        if (mListView != null) {
            ...
            if (htvView != null) {
                ...
                MsgEntity itemMessage = (MsgEntity) getItem(itemPosition);
    ...
    private Bitmap getThumbnailBitmap(MsgEntity msg) {
        String imagePath = NearChatApplication.THUMBNAIL_PATH + File.separator;
        if (SessionManager.isLocalUser(msg.getSenderIMEI())) {
    ...
    private String getThumbnailPath(MsgEntity msg) {
        StringBuffer imagePath = new StringBuffer();
        imagePath.append("file://").append(NearChatApplication.THUMBNAIL_PATH).append(File.separator);
        if (SessionManager.isLocalUser(msg.getSenderIMEI())) {
