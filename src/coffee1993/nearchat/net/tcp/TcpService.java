...
public class TcpService implements Runnable {
    ...
    private TcpService() {
        try {
            serviceSocket = new ServerSocket(Constant.TCP_SERVER_RECEIVE_PORT);//创建一个新的Socket
            saveFileToDisks = new ArrayList<TcpService.SaveFileToDisk>();
            NearChatLog.d(TAG, "建立监听服务器ServerSocket成功");
        }
        catch (IOException e) {
            // TODO Auto-generated catch block
            NearChatLog.d(TAG, "建立监听服务器ServerSocket失败");
            e.printStackTrace();
        }
    ...
    public void setSavePath(String fileSavePath) {
        NearChatLog.d(TAG, "设置存储路径成功,路径为" + fileSavePath);
    ...
    public void run() {
        ...
        NearChatLog.d(TAG, "TCP_Service线程开启");
    ...
    public class SaveFileToDisk extends Thread {
        ...
        public SaveFileToDisk(Socket socket) {
            try {
                ...
                NearChatLog.d(TAG, "获取网络输入流成功");
            }
            catch (IOException e) {
                // TODO Auto-generated catch block
                NearChatLog.d(TAG, "获取网络输入流失败");
                SCAN_RECIEVE = false;
                e.printStackTrace();
            }
        }
        ...
        public void recieveFile() {
            ...
            try {
                ...
                NearChatLog.d(TAG, "传输文件类型:" + strData[3]);
                ...
                NearChatLog.d(TAG, "文件存储路径:" + fileSavePath);
                FileState fileState = new FileState(length, 0, fileSavePath, getType(strData[3]));
                NearChatApplication.recieveFileStates.put(fileSavePath, fileState);
                FileState fs = NearChatApplication.recieveFileStates.get(fileSavePath);
                ...
                NearChatApplication.recieveFileStates.remove(fs.fileName);
            }
            catch (IOException e) {
                // TODO Auto-generated catch block
                NearChatLog.d(TAG, "写入文件失败");
                e.printStackTrace();
            }
        }

        private MsgEntity.CONTENT_TYPE getType(String string) {
        ...
        public void run() {
            super.run();
            NearChatLog.d(TAG, "SaveFileToDisk线程开启");
            if (SCAN_RECIEVE)
                recieveFile();
        }
    }
