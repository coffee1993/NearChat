package coffee1993.nearchat.sql;
...
public class SqlDBManager {
    private ChatDBHelper chatInfoSQLHelper; // 数据库类(t_chatting)
    private SQLiteDatabase chatInfoDataBase; // 数据库(t_chatting)的操作类
    private UserDBHelper userSQLHelper; // 数据库类(t_user)
    private SQLiteDatabase userDataBase;// 数据库(t_user)的操作类
    private static SqlDBManager sqlDBManager;
 
    /*
     * 构造函数参数：context对象通过db的方法来操作数据库的增删改查
     */
    public SqlDBManager(Context context) {
        chatInfoSQLHelper = new ChatDBHelper(context);
	...
	public static SqlDBManager getInstance(){
		if(sqlDBManager == null){
			synchronized (SqlDBManager.class) {
				if(sqlDBManager == null){
					sqlDBManager = new SqlDBManager(NearChatApplication.getContext());
				}		
			}
		}		
		return sqlDBManager;
    ...
    /* 获取在线信息尚未完善,默认在线状态(0) */
    public void addUserInfo(NearUser people) {
        ContentValues values = new ContentValues();
     ...
     * 用来获取近期的一系列聊天记录 参数:start为开始位置，count为最大记录数，(倒序排列) 放回List<chattingInfo>
     */
    public List<MsgEntity> getScrollMessageOfChattingInfo(int start, int count, int senderID,
            int recieverID) {
        List<MsgEntity> messages = new ArrayList<MsgEntity>();
        Cursor cursor = chatInfoDataBase.query(
                ...
                "id desc", start + "," + count);
        while (cursor.moveToNext()) {
            MsgEntity message = chattingInfoToMessage(new ChattingInfo(cursor.getInt(cursor
    ...
    private MsgEntity chattingInfoToMessage(ChattingInfo chattingInfo) {
