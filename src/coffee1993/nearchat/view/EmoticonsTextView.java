package coffee1993.nearchat.view;
...
public class EmoticonsTextView extends HandyTextView {
	...
	private Pattern buildPattern() {
	    List<String> cachemEmoticons = NearChatApplication.mEmoticons;
	...
	private CharSequence replace(CharSequence text) {
		try {
			...
			while (matcher.find()) {
				if (NearChatApplication.mEmoticonsId.containsKey(matcher.group())) {
					int id = NearChatApplication.mEmoticonsId.get(matcher.group());
