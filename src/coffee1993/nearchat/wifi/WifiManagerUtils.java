package coffee1993.nearchat.wifi;
...
public class WifiManagerUtils {
    ...
    private static Context mContext = NearChatApplication.getInstance();
    ...
    public static void startWifiAp(String ssid, String passwd, final Handler handler) {
        ...
        NearChatLog.i("startWifiAp"+ssid+" "+passwd);
        
        startAp(ssid, passwd);
        TimerCheck timerCheck = new TimerCheck() {
            @Override
            public void doTimerCheckWork() {
            	//创建热点后需要循环检测热点是否可用确定可用之后才改变UI	
                if (isWifiApEnabled()) {
    ...
    private static void startAp(String ssid, String passwd) {
        ...
        try {
            ...
            NearChatLog.i("创建热点 configuration: "+netConfig.toString());
   ...
   */
    public static boolean connectWifi(String SSID, String Password, WifiCipherType Type) {
        ...
        //TODO 继续调试 找到Bug
        if (tempConfig != null) {
        	NearChatLog.i("移除找到的wifi"+tempConfig.networkId);
    ...
    private static WifiConfiguration createWifiInfo(String SSID, String Password,
        ...
        if (Type == WifiCipherType.WIFICIPHER_WPA) {
        	...
        	NearChatLog.i("连接 专用wifi"+config.SSID+" "+config.SSID);
            config.preSharedKey = "\"" + Password + "\"";
            config.hiddenSSID = true;
        ...
        NearChatLog.i("连接wifi+WifiUtils createInfo :"+config.toString());
        return config;
    }
                ...
                if (!ni.isLoopback()) {
                    for (InterfaceAddress interfaceAddress : ni.getInterfaceAddresses()) {
                        if (interfaceAddress.getBroadcast() != null) {
                            NearChatLog.d(TAG, interfaceAddress.getBroadcast().toString().substring(1));
    ...
    private static WifiConfiguration isExsits(String SSID) {
    	...
    	NearChatLog.i("查找以前是否配置当前wifi："+SSID);
    	
        List<WifiConfiguration> existingConfigs = mWifiManager.getConfiguredNetworks();
        for (WifiConfiguration existingConfig : existingConfigs) {
            
        	NearChatLog.i("便利Wifi: "+existingConfig.SSID+": "+existingConfig.BSSID);
        	
        	if (existingConfig.SSID.equals("\"" + SSID + "\"")) {
            	
        		NearChatLog.i("找到了Wifi并返回： "+existingConfig.SSID+": "+existingConfig.BSSID);
